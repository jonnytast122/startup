	...
	return (
		<div className="relative flex items-center gap-2">
			...
			{showDatePicker && (
				<div
					ref={datePickerRef}
					className="fixed -translate-x-1/2 -translate-y-full bg-white shadow-2xl border p-2 rounded-md z-[9999]"
					style={{
						top: `${position.top}px`,
		...
		<div className="bg-white rounded-xl shadow-md py-6 px-4 sm:px-6 border mt-5 mb-10 w-full overflow-x-auto">
			...
			<div className="mb-2 mt-2 flex flex-col sm:flex-row gap-4 text-base font-custom">
				...
				<span>
					...
					{formatWorkHours(
						reduce(attendances, (sum, row) => sum + row.workHours, 0)
					)?.slice(0, 5)}{" "}
					hours
				</span>
			</div>
					...
					{attendances.map((row, i) => (
						<React.Fragment key={row.date}>
							...
							<TableRow className="text-sm font-custom">
								...
								<TableCell>
									{formatTime(
										row.transactions[row.transactions.length - 1].time
									)}
								</TableCell>

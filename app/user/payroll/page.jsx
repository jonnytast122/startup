...
const usdToComma = (n) => (n == null ? "--" : `$ ${n.toFixed(2).replace(".", ",")}`);
  ...
  return (
    <div className="grid grid-cols-[64px_1fr] items-center gap-4">
      <div className="text-center">
        <div className="text-[18px] leading-none text-gray-700 font-custom">{format(d, "d")}</div>
        <div className="text-xs text-gray-500 font-custom -mt-0.5">{format(d, "EEE")}</div>
      </div>

      {noteLeft || noteRight ? (
        <div className="py-1 text-sm font-custom">
          <span className="text-blue-500">{noteLeft}</span>
          {noteRight && <span className="float-right text-blue-500">{noteRight}</span>}
        </div>
      ) : (
        <div className="rounded-2xl bg-gray-100 px-6 py-4">
          <div className="grid grid-cols-3 text-center gap-3">
            <div>
              <div className="text-gray-500 text-sm font-custom">Work hours</div>
              <div className="text-[18px] font-semibold text-gray-800 font-custom">{minToHHMM(workMin)}</div>
            </div>
            <div>
              <div className="text-gray-500 text-sm font-custom">Break hours</div>
              <div className="text-[18px] font-semibold text-gray-800 font-custom">{minToHHMM(breakMin)}</div>
            </div>
            <div>
              <div className="text-gray-500 text-sm font-custom">Total paid</div>
              <div className="text-[18px] font-semibold text-gray-800 font-custom">{usdToComma(paidUsd)}</div>
            </div>
          </div>
        </div>
      )}
...
const WeekTotal = ({ minutes }) => (
  <div className="text-center my-2">
    <span className="font-custom text-gray-500">
      Week total <span className="text-gray-700 font-semibold">{minToHHMM(minutes)}</span>
    </span>
  </div>
);
...
export default function Payroll() {
  ...
  // SOURCE DATA (could come from API)
  const allRows = useMemo(
    () => [
      { date: "2025-09-29", workMin: 7 * 60 + 12, breakMin: 30, paidUsd: 5.12 },
      { date: "2025-09-28" },
      { date: "2025-09-27", workMin: 4 * 60 + 15, breakMin: 0, paidUsd: 4.12 },
      { date: "2025-09-27", workMin: 4 * 60 + 15, breakMin: 0, paidUsd: 4.12 },
      { date: "2025-09-26", workMin: 7 * 60 + 4, breakMin: 30, paidUsd: 5.12 },
      { date: "2025-09-25", noteBefore: { left: "Time off", right: "07:00 (paid)" }, workMin: 4 * 60 + 12, breakMin: 30, paidUsd: 4.02 },
      { date: "2025-09-24", workMin: 4 * 60 + 12, breakMin: 30, paidUsd: 4.02 },
      { date: "2025-09-23", workMin: 4 * 60 + 12, breakMin: 30, paidUsd: 4.02 },
      { date: "2025-09-22", workMin: 4 * 60 + 12, breakMin: 30, paidUsd: 4.02 },
    ],
    []
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white rounded-xl shadow-sm border p-6">
          ...
          <div className="flex-1">
            <div className="bg-[#4F8BDB] text-white rounded-2xl p-8 min-h-[260px] flex flex-col justify-center">
              ...
              <div className="grid grid-cols-2 gap-6 text-center">
                <div>
                  <div className="text-white/80 text-sm font-custom mb-1">Monthly rate</div>
                  <div className="text-2xl font-semibold font-custom tracking-wide">{monthlyRate} USD</div>
                </div>
                <div>
                  <div className="text-white/80 text-sm font-custom mb-1">Hourly rate</div>
                  <div className="text-2xl font-semibold font-custom tracking-wide">{hourlyRate.toFixed(2)} USD</div>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-[1.2] flex flex-col gap-6">
                  ...
                  >
                    <DateRangePicker
                      ...
                      onChange={(ranges) => {
                        ...
                        if (next.startDate && next.endDate && next.startDate.getTime() !== next.endDate.getTime()) {
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              <div className="rounded-2xl bg-gray-100 px-8 py-6 text-center">
                <div className="text-[44px] leading-none font-light font-custom text-[#3B8AFF]">{workedHours}</div>
                <div className="text-gray-500 font-custom text-lg -mt-1">/ {targetHours}</div>
              </div>
              <div className="rounded-2xl bg-gray-100 px-8 py-6 text-center">
                <div className="text-[44px] leading-none font-light font-custom text-[#FF8A4A]">{earnings}</div>
                <div className="text-gray-500 font-custom text-lg -mt-1">/ {earningsTarget}</div>
              </div>
            </div>
            ...
            <div className="mt-2">
              ...
              <div className="divide-y">
                <div className="py-3 grid grid-cols-[1fr_auto_auto] gap-3 items-center">
                  ...
                  <div className="text-gray-500 font-custom text-sm justify-self-end">{monthlyTaxPct}%</div>
                  <div className="text-gray-700 font-custom justify-self-end">{monthlyTaxUSD} USD</div>
                </div>
                <div className="py-3 grid grid-cols-[1fr_auto_auto] gap-3 items-center">
                  <div className="text-gray-600 font-custom">NSSF</div>
                  <div className="text-gray-500 font-custom text-sm justify-self-end" />
                  <div className="text-gray-700 font-custom justify-self-end">{nssfUSD} USD</div>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mt-10">
          <h2 className="text-center font-custom text-xl text-gray-700 mb-5">Payment sheets</h2>
                ...
                <div key={g.week} className={gi > 0 ? "mt-4" : ""}>
                  {g.days.map((d, i) => (
                    <div key={`${d.date}-${i}`} className={i > 0 ? "mt-3" : ""}>
                      {d.noteBefore ? (
                        <DayRow date={d.date} noteLeft={d.noteBefore.left} noteRight={d.noteBefore.right} />
                      ) : null}
                      <DayRow date={d.date} workMin={d.workMin} breakMin={d.breakMin} paidUsd={d.paidUsd} />
                    </div>
                  ))}

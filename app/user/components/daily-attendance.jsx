...
import { getDailyAttendance } from "@/lib/api/userReport";
...
export default function DailyAttendance() {
  ...
  const { data: dailyAttendance, isLoading } = useQuery({
    queryKey: ["daily-attendance", startOfMonth, endOfMonth],
    queryFn: () =>
      getDailyAttendance({ startDate: startOfMonth, endDate: endOfMonth }),
  });
  ...
  if (dailyAttendance?.data?.length > 0) {
    dailyAttendance.data.forEach((record) => {
      if (record.status === "On Time") onTime += 1;
      else if (record.status === "Late") runningLate += 1;
      else if (record.status === "Absent")
        onLeave += 1; // or use another field if leave
      else noActive += 1;
    });
  }
  ...
  const attendanceData = [
    ...
    { name: "Running Late", value: runningLate },
  ];

  const absentData = [
    { name: "No Active", value: noActive },

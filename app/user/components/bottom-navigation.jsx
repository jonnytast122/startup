...
import { User, Home, SquareKanban, Info } from "lucide-react";

const mainNavItems = [
  { title: "Home", url: "/user/attendance", icon: Home, alert: 0 },
          ...
          {mainNavItems.map((item) => {
            ...
            return (
              <Link
                key={item.title}
                href={item.url}
                className={`relative flex flex-col m-1 items-center justify-center px-2 py-2 rounded-lg min-w-0 flex-1 transition-colors ${
                  isActive

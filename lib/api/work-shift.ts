...
export const updateWorkShift = async ({id, data}: {id: string, data:any}) => {
  const response = await api.put(ApiRoutes.workShift.update.replace('{id}', id), data);
  return response.data;
}

export const deleteWorkShift = async (id: string) => {
  const response = await api.delete(ApiRoutes.workShift.delete.replace('{id}', id));
  return response.data;
}